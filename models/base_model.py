...
class BaseModel(ABC):
    ...
    def print_model_comparison(comparison_result):
        ...
        # Create table header
        print(f"{'Model':<20} {'AIC':<12} {'BIC':<12} {'AIC Weight':<12} {'Success':<8}")
        print("-"*70)
        
        # Sort by AIC for display
        details = sorted(comparison_result['model_details'], key=lambda x: x['aic'])
    ...
    def get_parameter_count(model_name, n_terms=None):
        ...
        if model_name == 'multipole_debye':
            ...
            return 3 * (n_terms or 2) + 1  # N*delta_eps + N*f_k + N*sigma_k + eps_inf
    ...
    def enhanced_model_comparison(results_dict, selection_method="balanced"):
        ...
        rationale = BaseModel.generate_selection_rationale(best_result, comparison_data, selection_method)
    ...
    def generate_selection_rationale(best_result, all_results, method):
        ...
        alternatives = [r for r in all_results if r['model'] != model_name and r['overall_rank'] <= 3]
        ...
        if alternatives:
            alt_names = [r['model'].replace('_', ' ').title() for r in alternatives[:2]]
    ...
    def print_enhanced_comparison(comparison_result):
        ...
        # Selection summary
        print(f"🏆 Best Model: {comparison_result['best_model'].replace('_', ' ').title()}")
        print(f"📊 Selection Method: {comparison_result['selection_method'].replace('_', ' ').title()}")
        print(f"💡 Rationale: {comparison_result['selection_rationale']}")
        print()
        
        # Detailed comparison table
        print(f"{'Model':<18} {'Rank':<5} {'AIC':<8} {'RMSE':<8} {'R²':<6} {'Params':<7} {'Status':<8}")
        print("-"*80)
        
        for item in comparison_result['comparison_data']:
            # Calculate R² from RMSE (approximate)
            rmse_normalized = item['rmse'] / max(0.1, np.mean([item.get('rmse_dk', 0), item.get('rmse_df', 0)]))
        ...
        print("-"*80)
        print(f"Summary: {summary['successful_fits']}/{summary['total_models']} successful fits")
        print(f"AIC range: {summary['aic_range'][0]:.1f} to {summary['aic_range'][1]:.1f}")
        print(f"RMSE range: {summary['rmse_range'][0]:.4f} to {summary['rmse_range'][1]:.4f}")
        print("="*80)
