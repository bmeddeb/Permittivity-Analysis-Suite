...
class ColeColeModel(BaseModel):
    ...
    def create_parameters(self, freq, dk_exp, df_exp):
        ...
        params.add('alpha', value=alpha_guess, min=0.0,
                   max=0.99)  # Cole-Cole constraint
    ...
    def analyze(self, df):
        ...
        if alpha_fit > 0.1:
            ...
            print(
                f"Relaxation time distribution factor: {distribution_factor:.1f}x broader than Debye")
        ...
        eps_fit_corrected = self.handle_negative_imaginary(
            eps_fit, "Cole-Cole model")

        # Extract fitted parameters in expected format
        fitted_params = self.extract_fitted_params(result, "cole_cole")
