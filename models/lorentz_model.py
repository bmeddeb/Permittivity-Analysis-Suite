...
class LorentzModel(BaseModel):
    ...
    def create_parameters(self, freq, dk_exp, df_exp):
        ...
        gamma_initial = [w0 * 0.1 for w0 in w0_initial]  # 10% of resonance frequency
        ...
        for i in range(self.N):
            ...
            params.add(f'w0_{i}', value=w0_initial[i], min=freq_min / 100, max=freq_max * 100)
            params.add(f'gamma_{i}', value=gamma_initial[i], min=freq_min / 1000, max=freq_max * 10)
    ...
    def analyze(self, df):
        ...
        print(f"Initial guess - f: {[params[f'f_{i}'].value for i in range(self.N)]}")
        print(f"Initial guess - w0 (rad/s): {[params[f'w0_{i}'].value for i in range(self.N)]}")
        print(f"Initial guess - gamma (rad/s): {[params[f'gamma_{i}'].value for i in range(self.N)]}")
        ...
        print(f"Bounds - w0: [{params['w0_0'].min:.2e}, {params['w0_0'].max:.2e}] rad/s")
        print(f"Bounds - gamma: [{params['gamma_0'].min:.2e}, {params['gamma_0'].max:.2e}] rad/s")
        print(f"Bounds - eps_inf: [{params['eps_inf'].min:.1f}, {params['eps_inf'].max:.3f}]")
        ...
        gamma_fitted = [result.params[f'gamma_{i}'].value for i in range(self.N)]
        eps_inf_fitted = result.params['eps_inf'].value
        
        print(f"Fitted - f: {f_fitted}")
        print(f"Fitted - w0 (rad/s): {w0_fitted}")
        print(f"Fitted - w0 (GHz): {[w0 / (2 * np.pi * 1e9) for w0 in w0_fitted]}")
        ...
        quality_factors = [w0 / gamma for w0, gamma in zip(w0_fitted, gamma_fitted)]
