...
class HybridModel(BaseModel):
    ...
    def create_parameters(self, freq, dk_exp, df_exp):
        ...
        for i in range(self.N):
            params.add(
                f'delta_eps_{i}', value=delta_eps_initial, min=0.0, max=dk_range * 3)
            params.add(
                f'f_k_{i}', value=f_k_initial[i], min=freq_min / 100, max=freq_max * 100)
            params.add(f'sigma_k_{i}',
                       value=sigma_k_initial, min=0.0, max=10.0)
    ...
    def model_function(self, params, freq):
        ...
        sigma_k = np.array(
            [params[f'sigma_k_{i}'].value for i in range(self.N)])
    ...
    def analyze(self, df):
        ...
        print(
            f"Bounds - f_k: [{params['f_k_0'].min:.2e}, {params['f_k_0'].max:.2e}] GHz")
        ...
        sigma_k_fitted = [
            result.params[f'sigma_k_{i}'].value for i in range(self.N)]
        ...
        print(
            f"Dominant Debye term: {dominant_debye} at {f_k_fitted[dominant_debye]:.2f} GHz")
        print(
            f"Dominant Lorentz term: {dominant_lorentz} at {f_k_fitted[dominant_lorentz]:.2f} GHz")

        # Check if terms are well-separated
        f_k_sorted = np.sort(f_k_fitted)
        if self.N > 1:
            min_separation = np.min(
                [f_k_sorted[i+1] / f_k_sorted[i] for i in range(self.N-1)])
            print(f"Minimum frequency separation factor: {min_separation:.2f}")
            if min_separation < 2:
                print(
                    "Warning: Some relaxation frequencies are very close - consider reducing N")

        # Handle negative imaginary parts using BaseModel method
        eps_fit_corrected = self.handle_negative_imaginary(
            eps_fit, "Hybrid model")

        # Create params_fit array for compatibility
        params_fit = delta_eps_fitted + f_k_fitted + \
            sigma_k_fitted + [eps_inf_fitted]

        # Extract fitted parameters in expected format
        fitted_params = self.extract_fitted_params(result, "hybrid")
