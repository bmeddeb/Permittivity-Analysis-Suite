...
def index():
    ...
    if current_user.is_authenticated:
        recent_trials = (
            current_user.trials.order_by(Trial.upload_timestamp.desc()).limit(5).all()
        )

    return render_template(
        "index.html", title="Home", form=form, recent_trials=recent_trials
    )
...
def process_csv_upload(form):
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    flash(
                        "Unable to decode file. Please ensure it is a valid CSV file with UTF-8 or standard encoding.",
                        "error",
                    )
        ...
        actual_headers = set(
            header.strip() for header in csv_reader.fieldnames if header
        )
        expected_headers = {"Frequency_GHz", "Dk", "Df"}

        if not expected_headers.issubset(actual_headers):
            flash(
                f"Invalid CSV format. Expected headers: {', '.join(expected_headers)}. Found: {', '.join(actual_headers)}",
                "error",
            )
            return redirect(url_for("main.index"))
        ...
        for row in csv_reader:
            ...
            try:
                ...
                flash(
                    f"Error processing row {row_count + 1}: {str(e)}. Row data: {dict(row)}",
                    "error",
                )
                return redirect(url_for("main.index"))
        ...
        if filename_duplicate:
            flash(
                f'A file with the name "{filename}" has already been uploaded on {filename_duplicate.upload_timestamp.strftime("%Y-%m-%d %H:%M")}. Please rename your file or use a different filename.',
                "warning",
            )
            return redirect(url_for("main.index"))
        ...
        if content_duplicate:
            flash(
                f'This data has already been uploaded as "{content_duplicate.filename}" on {content_duplicate.upload_timestamp.strftime("%Y-%m-%d %H:%M")}. The content is identical even though the filename may be different.',
                "warning",
            )
            return redirect(url_for("main.index"))
        ...
        flash(
            f"Successfully uploaded {filename} with {row_count} data points!", "success"
        )
        return redirect(url_for("main.index"))
...
def dashboard_gridstack():
    ...
    trials = (
        ...
        .order_by(Trial.upload_timestamp.desc())
        .all()
    )
    ...
    try:
        ...
        if new_filename and new_filename != trial.filename:
            ...
            existing_trial = Trial.query.filter_by(
                user_id=current_user.id, filename=new_filename
            ).first()
        ...
        return jsonify(
            {
                ...
                "trial": {"id": trial.id, "filename": trial.filename},
            }
        )
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error updating trial: {str(e)}"}
        ), 500
...
def update_material_data(data_id):
    material_data = (
        MaterialData.query.join(Trial)
        .filter(MaterialData.id == data_id, Trial.user_id == current_user.id)
        .first_or_404()
    )
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error updating data: {str(e)}"}
        ), 500
...
def delete_material_data(data_id):
    ...
    try:
        ...
        if remaining_data:
            ...
            trial.data_fingerprint = hashlib.sha256(
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error deleting data: {str(e)}"}
        ), 500
...
def add_material_data(trial_id):
    ...
    try:
        ...
        existing_data = MaterialData.query.filter_by(
            trial_id=trial_id, frequency_ghz=frequency_ghz
        ).first()
        ...
        new_data = MaterialData(
            frequency_ghz=frequency_ghz, dk=dk, df=df, trial_id=trial_id
        )
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error adding data: {str(e)}"}
        ), 500
...
def download_trial_csv(trial_id):
    ...
    for data in material_data:
        writer.writerow(
            [f"{data.frequency_ghz:.6f}", f"{data.dk:.6f}", f"{data.df:.8f}"]
        )
