...
class Trial(db.Model):
    ...
    data_fingerprint = db.Column(
        db.String(64), nullable=True, index=True
    )  # SHA256 hash of data content

    # Relationship to material data and fitted models
    material_data = db.relationship(
        "MaterialData", backref="trial", lazy="dynamic", cascade="all, delete-orphan"
    )
    fitted_models = db.relationship(
        "FittedModel", backref="trial", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class FittedModel(db.Model):
    ...
    # Fit results
    fit_parameters = db.Column(
        db.JSON, nullable=False
    )  # Fitted parameter values and uncertainties
    fit_quality_metrics = db.Column(
        db.JSON, nullable=False
    )  # R-squared, chi-squared, etc.
    fitted_frequency_data = db.Column(
        db.JSON, nullable=False
    )  # Frequency points for fitted curve
    ...
    model_config = db.Column(
        db.JSON, nullable=True
    )  # Model-specific parameters (e.g., number of poles)
