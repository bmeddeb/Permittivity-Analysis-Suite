...
class SarkarExportUtils:
    ...
    def generate_spice_model(self, params: lmfit.Parameters,
                             z0: float = 50.0,
                             format: str = 'behavioral') -> Dict[str, Any]:
        """
    ...
    def _generate_behavioral_netlist(self, params: Dict[str, float],
                                     z0: float) -> List[str]:
        ...
        netlist.append("G1 IN OUT LAPLACE {V(IN,REF)} = " +
                       self._generate_laplace_expression(params))
        netlist.append(".ENDS DS_MODEL")
        return netlist

    def _calculate_rc_ladder(self, params: Dict[str, float],
                             n_sections: int = 5) -> Dict[str, Any]:
    ...
    def _generate_ladder_netlist(self, ladder_params: Dict[str, Any],
                                 z0: float) -> List[str]:
    ...
    def to_touchstone(self, params: lmfit.Parameters,
                      freq_hz: np.ndarray,
                      thickness_m: float = 1e-3,
                      z0: float = 50.0,
                      eps_r_ref: float = 1.0) -> Dict[str, np.ndarray]:
        """
    ...
    def generate_touchstone_file(self, params: lmfit.Parameters,
                                 freq_hz: np.ndarray,
                                 filename: str,
                                 thickness_m: float = 1e-3,
                                 z0: float = 50.0,
        ...
        with open(filename, 'w') as f:
            ...
            f.write(f"! f1: {p['f1']:.3e} " + ("GHz" if self.model.use_ghz else "Hz") + "\n")
            f.write(f"! f2: {p['f2']:.3e} " + ("GHz" if self.model.use_ghz else "Hz") + "\n")
            ...
            for i, freq in enumerate(freq_hz):
                if format == 'RI':
                    ...
                    f.write(f"{s_data['s22'][i].real:.6e} {s_data['s22'][i].imag:.6e}\n")
                else:  # MA format
                    f.write(f"{freq:.6e} ")
    ...
    def extrapolate_with_uncertainty(self, params: lmfit.Parameters,
                                     freq_extrap: np.ndarray,
                                     freq_data: np.ndarray,
                                     confidence: float = 0.95,
                                     extrapolation_penalty: float = 0.1) -> Dict[str, np.ndarray]:
        """
        ...
        distance = np.maximum(0, np.maximum(log_data_min - log_freq,
                                            log_freq - log_data_max))
        ...
        if max_distance > 2:
            result['warning'] = f"Extreme extrapolation: {max_distance:.1f} decades beyond data"
        elif max_distance > 1:
            result['warning'] = f"Significant extrapolation: {max_distance:.1f} decades beyond data"
    ...
    def generate_plot_data(self, result: lmfit.model.ModelResult,
                           n_points: int = 1000,
        ...
        plot_data['df_residual'] = result.df_exp - eps_fit_at_data.imag / eps_fit_at_data.real
        ...
        plot_data['f_center'] = np.sqrt(p['f1'] * p['f2']) if self.model.use_ghz else \
            np.sqrt(p['f1'] * p['f2']) / 1e9
        ...
        in_data_range = (freq_plot >= result.freq.min()) & (freq_plot <= result.freq.max())
        plot_data['is_interpolation'] = in_data_range
        plot_data['is_extrapolation'] = ~in_data_range

        return plot_data

    def _calculate_components(self, freq: np.ndarray,
                              params: lmfit.Parameters) -> Dict[str, np.ndarray]:
